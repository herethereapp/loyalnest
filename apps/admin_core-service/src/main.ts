...
async function bootstrap() {
  ...
  const serviceName = 'admin-core-service';
  const servicePort = parseInt(process.env.PORT || '3008', 10); // Matches docker-compose.yml
  const serviceAddress = 'admin-core';
  ...
  try {
    await consul.agent.service.register({
      ...
      port: servicePort,
      check: {
        name: `${serviceName}-health`, // Unique health check name
        http: `http://${serviceAddress}:${servicePort}/${globalPrefix}/health`,
        interval: '10s',
        timeout: '5s',
        deregistercriticalserviceafter: '30s', // Lowercase per Consul API
      },
    });
